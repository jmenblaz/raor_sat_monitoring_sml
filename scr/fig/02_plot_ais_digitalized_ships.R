#------------------------------------------------------------------------------
# 04. Plot - Difference in AIS, Camera Monitoring and digitized ships
#--------------------------------------------------------------------------
...
# for plot
ships <- ships %>% filter(length_m <60)
# prepare data
ais_df <- data.frame(length = ais$length,
                     method = "ais")

ship_df <- data.frame(length = ships$length_m,
                      method = "satellite")
# combine dfs
...
p <- ggplot(data = df, mapping = aes(x = method, y = length, fill = method)) +
  ...
  sm_raincloud(data = df_ais, position = position_nudge(x = -0.05),
               ...
               point.params = list(size = 1, shape = 21,
                                   ...
                                   show.legend = FALSE,
                                   alpha = 0.5,
                                   position = sdamr::position_jitternudge(nudge.x = +0.15, seed = 15,
                                                                          jitter.width = 0.20))) +
  ...
  sm_raincloud(data = df_sat, position = position_nudge(x = +0.05),
               ...
               show.legend = FALSE,
               vertical = TRUE,
               point.params = list(size = 0.3, shape = 21,
                                   colour = "orange", 
                                   show.legend = FALSE,
                                   alpha = 0.06,
                                   position = sdamr::position_jitternudge(nudge.x = -0.15, seed = 15,
...
ggsave(p_svg, p, width=8, height=8, units="cm", dpi=450, bg="white")
...
p1 <- basemap + 
  ...
  geom_point(data = ais, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]),
             color = "lightblue", size = 3.4, alpha = 0.15) +
  geom_point(data = ais, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]),
             color = "grey15", size = 2.7, alpha = 1) +
  geom_point(data = ais, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]),
             color = "#87CEFA", size = 1.5, alpha = 0.9) + 
  # plot info
...
p2 <- basemap +
  ...
  geom_point(data = ships, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]),
             color = "#FFA54F", size = 3.4, alpha = 0.1) +
  geom_point(data = ships, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]),
             color = "grey15", size = 2.7, alpha = 0.8) +
  geom_point(data = ships, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]),
...
# P3 - Total Cloudpoint Camera tracking IMEDEA
# Change ships by IMEDEA points ***********************************************
p3 <- basemap +
  # vessels points
  geom_point(data = ships, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]),
             color = "#9ACD32", size = 3.4, alpha = 0.1) +
  geom_point(data = ships, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]),
             color = "grey15", size = 2.7, alpha = 0.8) +
  geom_point(data = ships, aes(x = st_coordinates(geometry)[, 1], y = st_coordinates(geometry)[, 2]),
             color = "#A9E037", size = 1.5, alpha = 0.8) +
  
  # plot info
  annotate("text", x = 2.745, y = 39.4635, label = paste0("Fixed Camera"), parse = FALSE, family = "Arial",
           size = 4, color = "snow1") +
  annotate("text", x = 2.748, y = 39.4615, label = paste0("italic(n) == ", nship), parse = TRUE, family = "Arial",
           size = 3.5, color = "snow1") +
  annotate("pointrange", x = 2.7360, y = 39.4635, ymin = 12, ymax = 28,
           colour = "#9ACD32", size = 1.3, linewidth = 0.75, alpha = 0.2) +
  annotate("pointrange", x = 2.7360, y = 39.4635, ymin = 12, ymax = 28,
           colour = "#000000", size = 1.1, linewidth = 0.75) +
  annotate("pointrange", x = 2.7360, y = 39.4635, ymin = 12, ymax = 28,
           colour = "#A9E037", size = 0.8, linewidth = 0.75)
...
ggsave(p_png, p, width = 30, height = 12, units="cm", dpi=350, bg="white")
ggsave(p_svg, p, width = 30, height = 12, units="cm", dpi=350, bg="white")
